...
TempoIQ.Client = function(key, secret, host, opts) {
  ...
  base.listDevices = function(selection, options, callback) {
    ...
    var withOptions = function(sel, opt, cb) {
      ...
      var cursor = new Cursor(Device, base._session, "/v2/devices", query, {
        "Accept": "application/prs.tempoiq.device-collection.v2+json"
      });
      if (opt.streamed) {
        cb(cursor);
      } else {
  ...
  base.read = function(selection, start, end, pipeline, options, callback) {
    ...
    var withOptions = function(sel, st, en, pip, opt, cb) {
      ...
      var cursor = new Cursor(Row, base._session, "/v2/read", query, {
        "Accept": "application/prs.tempoiq.datapoint-collection.v2+json"
      });
      if (opt.streamed) {
        cb(cursor);
      } else {
  ...
  base.latest = function(selection, pipeline, options, callback) {
    ...
    var withOptions = function(sel, pip, opt, cb) {
      var query = new Query(selection, pipeline, {name: "single", value: {}});
    ...
    if (arguments.length == 3) {
      withOptions(selection, pipeline, {streamed: false}, options);
    } else {
      withOptions(selection, pipeline, options, callback);
    }
  };
  _initialize();
  return base;
